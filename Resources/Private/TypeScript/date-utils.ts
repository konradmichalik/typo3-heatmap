 ...
 */
export function getWeekStart(date: Date, weekStartsOnMonday: boolean): Date {
 ...
 */
export function weekSpan(start: Date, end: Date, weekStartsOnMonday: boolean): number {
 ...
 */
export function resolveWeekCount(containerWidth: number, mode: DateRangeMode): number {
    if (mode === 'year') return 53;   // ~1 year
    if (mode === 'month') return 5;   // ~1 month

    // 'auto' (and any width-driven fallback): breakpoints on container width.
    if (containerWidth >= 700) return 53; // 1 year
    if (containerWidth >= 400) return 26; // 6 months
    return 13;                            // 3 months
}
